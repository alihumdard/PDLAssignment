<?php
    ...
    // Check if there are related records in the department table
    $checkPersonnelQuery = $conn->prepare('SELECT COUNT(*) FROM department WHERE locationID = ?');
    ...
    if ($personnelCount > 0) {
        ...
        $output['status']['description'] = "Cannot delete location with related deparment records. Delete/edit related deparment first.";
        $output['data'] = [];
        mysqli_close($conn);
        echo json_encode($output);
        exit;
